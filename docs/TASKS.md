...
## 📋 Phase 23: `beetle-app` 인게임 로딩 화면 (`AppScreen::Loading`) & 비동기 사운드뱅크 적재
- [ ] **`AppScreen::Loading` 전용 로딩 화면 구현**
  - [ ] 선곡 후 즉시 로딩 화면으로 전환 (곡명, 아티스트, BGA 아트워크, 로딩 인디케이터)
- [ ] **백그라운드 키음 디코딩 & 오디오 엔진 준비 파이프라인**
  - [ ] UI 스레드 블로킹 없이 수백 개 키음 사전 적재 완료 후 `Gameplay`로 부드럽게 전환
