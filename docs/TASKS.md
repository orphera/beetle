...
## 📋 Phase 15: 정적 BGA 및 타이틀 이미지 지원 (`#STAGEFILE` / `#BMP`)
- [ ] **순수 Rust 경량 BMP 디코더 구현 (`beetle-render`)**
  - [ ] 24-bit / 32-bit BMP 무외부 의존성 파서 구현
- [ ] **선곡창 STAGEFILE 썸네일 표출**
  - [ ] 선곡 정보 패널에 곡별 대표 아트워크 박스 렌더링
- [ ] **인게임 BGA 이미지 액자 렌더링**
  - [ ] 스펙트럼 비주얼라이저 상단에 곡 아트워크 표출
- [ ] **BMP 디코딩 단위 테스트 작성**

---

## 📋 Phase 16: 선곡창 곡 미리듣기 (Preview Audio Loop)
- [ ] **선곡창 프리뷰 오디오 루프 시스템 (`beetle-app`)**
  - [ ] 커서 정지 0.25초 후 해당 곡의 BGM/프리뷰 샘플 자동 재생
  - [ ] 커서 이동 시 즉시 이전 프리뷰 정지 및 새 곡 전환
- [ ] **안전한 오디오 리소스 전환**
  - [ ] 게임플레이 진입 시 프리뷰 정지 후 인게임 사운드뱅크로 전환

---

## 📋 Phase 17: 인게임 타건 파티클 & 콤보 펄스 애니메이션
- [ ] **판정선 히트 파티클 버스트 시스템 (`SoftwareRenderer`)**
  - [ ] 노트 타건 시 2D 빛 스파크 / 퍼짐 파티클 애니메이션
- [ ] **콤보 카운터 탄성 바운스(Scale Pulse)**
  - [ ] 콤보 증가 시 텍스트 팝업 펄스 모션
- [ ] **PGREAT 플래시 효과 고도화**

---

## 📋 Phase 18: 5키 / 9키 / 14키(DP) 다중 모드 지원
- [ ] **BMS 채보 모드 감지 (`PlayMode`: 5K, 7K, 9K, 14K)**
  - [ ] `#PLAYER 1/2/3` 및 레인 번호 기반 자동 판별
- [ ] **가변 플레이필드 레이아웃 및 렌더러 분기**
- [ ] **모드별 기본 키 프리셋 제공**
