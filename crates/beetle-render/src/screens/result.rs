...
impl SoftwareRenderer {
    ...
    pub fn render_result(
        ...
        let right_title_x = (vp.x + vp.width - BitmapFont::text_width(&title_str, font_scale) as f32 - 24.0 * s) as i32;
        BitmapFont::draw_text(&mut self.pixmap.as_mut(), &title_str, right_title_x, (vp.y + 10.0 * s) as i32, font_scale, ColorRgba::new(255, 255, 255, 255));
        let right_artist_x = (vp.x + vp.width - BitmapFont::text_width(&artist_str, font_scale) as f32 - 24.0 * s) as i32;
        BitmapFont::draw_text(&mut self.pixmap.as_mut(), &artist_str, right_artist_x, (vp.y + 26.0 * s) as i32, font_scale, ColorRgba::new(140, 150, 175, 255));
        ...
        self.draw_rect(banner_x, banner_y + 35.0 * s, banner_w, 1.0 * s, status_color);
        let banner_font_scale = (2.0 * s).round().max(1.0) as u32;
        BitmapFont::draw_text_centered(&mut self.pixmap.as_mut(), status_text, (vp.x + vp.width / 2.0) as i32, (banner_y + 8.0 * s) as i32, banner_font_scale, status_color);
        ...
        self.draw_rect(left_x, card_y, left_w, 1.0 * s, ColorRgba::new(35, 48, 75, 255));
        self.draw_rect(left_x, card_y + card_h - 1.0 * s, left_w, 1.0 * s, ColorRgba::new(35, 48, 75, 255));
        self.draw_rect(left_x, card_y, 1.0 * s, card_h, ColorRgba::new(35, 48, 75, 255));
        self.draw_rect(left_x + left_w - 1.0 * s, card_y, 1.0 * s, card_h, ColorRgba::new(35, 48, 75, 255));
        ...
        self.draw_rect(emblem_x, emblem_y + emblem_h - 2.0 * s, emblem_w, 2.0 * s, rank_glow);
        self.draw_rect(emblem_x, emblem_y, 2.0 * s, emblem_h, rank_glow);
        self.draw_rect(emblem_x + emblem_w - 2.0 * s, emblem_y, 2.0 * s, emblem_h, rank_glow);

        let rank_font_scale = (4.0 * s).round().max(2.0) as u32;
        BitmapFont::draw_text_centered(&mut self.pixmap.as_mut(), rank_str, (left_x + left_w / 2.0) as i32, (emblem_y + 16.0 * s) as i32, rank_font_scale, rank_color);
        ...
        // EX Score
        BitmapFont::draw_text(&mut self.pixmap.as_mut(), "EX SCORE", pad_x as i32, cur_y as i32, font_scale, ColorRgba::new(140, 150, 175, 255));
        cur_y += 16.0 * s;
        let ex_val = format!("{} / {}", score.ex_score, score.max_ex_score());
        BitmapFont::draw_text(&mut self.pixmap.as_mut(), &ex_val, pad_x as i32, cur_y as i32, num_scale, ColorRgba::new(255, 230, 80, 255));
        if let Some(prev) = previous_best {
            ...
            let diff_x = (left_x + left_w - BitmapFont::text_width(&diff_str, font_scale) as f32 - 24.0 * s) as i32;
            BitmapFont::draw_text(&mut self.pixmap.as_mut(), &diff_str, diff_x, (cur_y + 4.0 * s) as i32, font_scale, diff_col);
        }
        cur_y += 30.0 * s;

        // Accuracy
        BitmapFont::draw_text(&mut self.pixmap.as_mut(), "ACCURACY", pad_x as i32, cur_y as i32, font_scale, ColorRgba::new(140, 150, 175, 255));
        cur_y += 16.0 * s;
        let acc_val = format!("{:.2}%", score.accuracy_rate());
        BitmapFont::draw_text(&mut self.pixmap.as_mut(), &acc_val, pad_x as i32, cur_y as i32, num_scale, ColorRgba::new(80, 220, 255, 255));
        if let Some(prev) = previous_best {
        ...
        // Max Combo
        BitmapFont::draw_text(&mut self.pixmap.as_mut(), "MAX COMBO", pad_x as i32, cur_y as i32, font_scale, ColorRgba::new(140, 150, 175, 255));
        cur_y += 16.0 * s;
        let combo_val = format!("{} / {}", score.max_combo, score.total_notes);
        BitmapFont::draw_text(&mut self.pixmap.as_mut(), &combo_val, pad_x as i32, cur_y as i32, num_scale, ColorRgba::new(255, 255, 255, 255));
        if let Some(prev) = previous_best {
        ...
        self.draw_rect(mid_x, card_y, mid_w, 1.0 * s, ColorRgba::new(35, 48, 75, 255));
        self.draw_rect(mid_x, card_y + card_h - 1.0 * s, mid_w, 1.0 * s, ColorRgba::new(35, 48, 75, 255));
        self.draw_rect(mid_x, card_y, 1.0 * s, card_h, ColorRgba::new(35, 48, 75, 255));
        self.draw_rect(mid_x + mid_w - 1.0 * s, card_y, 1.0 * s, card_h, ColorRgba::new(35, 48, 75, 255));

        let mut mid_y = card_y + 20.0 * s;
        BitmapFont::draw_text(&mut self.pixmap.as_mut(), "JUDGE BREAKDOWN", (mid_x + 20.0 * s) as i32, mid_y as i32, font_scale, ColorRgba::new(160, 175, 205, 255));
        mid_y += 24.0 * s;
        ...
        for (label, count, color) in judge_counts {
            self.draw_rect(mid_x + 20.0 * s, mid_y, mid_w - 40.0 * s, 26.0 * s, ColorRgba::new(20, 25, 38, 200));
            BitmapFont::draw_text(&mut self.pixmap.as_mut(), label, (mid_x + 28.0 * s) as i32, (mid_y + 6.0 * s) as i32, font_scale, color);
            let cnt_str = format!("{:>5}", count);
            let cnt_x = (mid_x + mid_w - BitmapFont::text_width(&cnt_str, font_scale) as f32 - 28.0 * s) as i32;
            BitmapFont::draw_text(&mut self.pixmap.as_mut(), &cnt_str, cnt_x, (mid_y + 6.0 * s) as i32, font_scale, ColorRgba::new(255, 255, 255, 255));
            mid_y += 32.0 * s;
        }
        ...
        // Fast box
        self.draw_rect(mid_x + 20.0 * s, mid_y, fs_w, 40.0 * s, ColorRgba::new(15, 30, 50, 255));
        self.draw_rect(mid_x + 20.0 * s, mid_y, fs_w, 1.0 * s, ColorRgba::new(40, 100, 180, 255));
        BitmapFont::draw_text(&mut self.pixmap.as_mut(), "FAST", (mid_x + 28.0 * s) as i32, (mid_y + 6.0 * s) as i32, font_scale, ColorRgba::new(80, 200, 255, 255));
        let fast_str = format!("{}", score.fast_count);
        BitmapFont::draw_text(&mut self.pixmap.as_mut(), &fast_str, (mid_x + 28.0 * s) as i32, (mid_y + 20.0 * s) as i32, font_scale, ColorRgba::new(255, 255, 255, 255));

        // Slow box
        let slow_box_x = mid_x + 20.0 * s + fs_w + 8.0 * s;
        self.draw_rect(slow_box_x, mid_y, fs_w, 40.0 * s, ColorRgba::new(50, 25, 15, 255));
        self.draw_rect(slow_box_x, mid_y, fs_w, 1.0 * s, ColorRgba::new(180, 80, 40, 255));
        BitmapFont::draw_text(&mut self.pixmap.as_mut(), "SLOW", (slow_box_x + 10.0 * s) as i32, (mid_y + 6.0 * s) as i32, font_scale, ColorRgba::new(255, 140, 60, 255));
        let slow_str = format!("{}", score.slow_count);
        BitmapFont::draw_text(&mut self.pixmap.as_mut(), &slow_str, (slow_box_x + 10.0 * s) as i32, (mid_y + 20.0 * s) as i32, font_scale, ColorRgba::new(255, 255, 255, 255));
        ...
        self.draw_rect(right_x, card_y, right_w, 1.0 * s, ColorRgba::new(35, 48, 75, 255));
        self.draw_rect(right_x, card_y + card_h - 1.0 * s, right_w, 1.0 * s, ColorRgba::new(35, 48, 75, 255));
        self.draw_rect(right_x, card_y, 1.0 * s, card_h, ColorRgba::new(35, 48, 75, 255));
        self.draw_rect(right_x + right_w - 1.0 * s, card_y, 1.0 * s, card_h, ColorRgba::new(35, 48, 75, 255));

        let mut right_y = card_y + 20.0 * s;
        BitmapFont::draw_text(&mut self.pixmap.as_mut(), "TIMING OFFSET DISTRIBUTION", (right_x + 20.0 * s) as i32, right_y as i32, font_scale, ColorRgba::new(160, 175, 205, 255));
        right_y += 30.0 * s;
        ...
        self.draw_rect(hist_x, hist_y, hist_w, 1.0 * s, ColorRgba::new(35, 45, 68, 255));
        self.draw_rect(hist_x, hist_y + hist_h, hist_w, 1.0 * s, ColorRgba::new(35, 45, 68, 255));

        // Center line (0ms target)
        let center_hist_x = hist_x + hist_w / 2.0;
        self.draw_rect(center_hist_x, hist_y, 1.0 * s, hist_h, ColorRgba::new(80, 200, 255, 120));
        ...
        BitmapFont::draw_text(&mut self.pixmap.as_mut(), "-40ms (FAST)", hist_x as i32, label_y, font_scale, ColorRgba::new(80, 180, 240, 255));
        BitmapFont::draw_text_centered(&mut self.pixmap.as_mut(), "0ms (PERFECT)", center_hist_x as i32, label_y, font_scale, ColorRgba::new(255, 230, 80, 255));
        let slow_lbl_x = (hist_x + hist_w - BitmapFont::text_width("+40ms (SLOW)", font_scale) as f32) as i32;
        BitmapFont::draw_text(&mut self.pixmap.as_mut(), "+40ms (SLOW)", slow_lbl_x, label_y, font_scale, ColorRgba::new(255, 140, 60, 255));
